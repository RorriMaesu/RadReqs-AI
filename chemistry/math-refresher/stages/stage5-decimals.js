        ...
        mount({ host, state, onStateChange }) {
            ...
            host.innerHTML = `
                <style>
                    ...
                    .s5-pane { border: 1px solid rgba(255, 255, 255, 0.06); border-radius: 12px; background: rgba(15, 23, 42, 0.5); padding: 0.8rem; color: #f8fafc; }
                    ...
                    /* Gridboard */
                    .s5-gridboard { margin-top: 0.6rem; border: 1px solid rgba(255, 255, 255, 0.06); border-radius: 12px; padding: 0.6rem; background: rgba(15, 23, 42, 0.5); }
                ...
                <section class="s5-wrap">
                    ...
                    <article class="s5-card s5-level ${levelLocked(state.concreteUnlocked)}">
                        <h2>Concrete Level: Decimal Grid Alignment</h2>
                        <p><strong>L5.2 Decimals:</strong> Click digits in rows below to cycle them until the decimal points align vertically. Zeros can fill the empty slots.</p>
                        ...
                        <div class="s5-feedback">${state.decimalFeedback}</div>
                        ...
                        <div class="s5-pane" style="margin-top:0.75rem;">
                            ...
                            <div class="s5-status" style="margin-top:0; margin-bottom:0.5rem;">
                                <span class="s5-pill ${state.concreteMission.alignedDecimals ? 'good' : 'locked'}">Align both decimal points in one column</span>
                            </div>
                    ...
                    <article class="s5-card s5-level ${levelLocked(state.pictorialUnlocked)}">
                        <h2>Pictorial Level: Decimal Hopping</h2>
                        <p><strong>L5.3 Decimal Hopping:</strong> The product of 3.2 x 0.45 has 3 decimal places total. Hop the decimal point into 144 to represent 1.44.</p>
                        <div class="s5-pane">
                        ...
                        <div class="s5-feedback">${state.hopFeedback}</div>
                    ...
                    <article class="s5-card s5-level ${levelLocked(state.abstractUnlocked)}">
                        <h2>Abstract Level: Significant Figures Classification</h2>
                        <p><strong>L5.7 Precision Rules:</strong> Classify the zeros in 0.05080 to count significant figures. (Select: significant or not_significant).</p>
                        <div class="s5-grid">
                            <div class="s5-pane">
                                <strong>Leading Zeros (0.0...)</strong>
                                <select id="s5-sig-leading" class="s5-input" ${disabled(state.abstractUnlocked)}>
                            ...
                            <div class="s5-pane">
                                ...
                                <select id="s5-sig-trapped" class="s5-input" ${disabled(state.abstractUnlocked)}>
                                    <option value="">Choose...</option>
                            ...
                            <div class="s5-pane">
                                ...
                                <select id="s5-sig-trailing" class="s5-input" ${disabled(state.abstractUnlocked)}>
                                    <option value="">Choose...</option>
                    ...
                    <article class="s5-card s5-level ${levelLocked(state.appliedUnlocked)}">
                        <h2>Applied Level: Lab Percent Error</h2>
                        <p><strong>[Required Unlock] L5.6 Applied:</strong> A student measures a sample mass of 9.0 g. The true reference standard weight is 10.0 g. What is the calculated percent error of this lab trial?</p>
                        <div class="s5-grid">
                        ...
                        <div class="s5-feedback">${state.appliedFeedback}</div>

                        <div class="s5-pane" style="margin-top:0.75rem;">
                            <p><strong>[Reinforcement] L5.6 Absolute vs Relative Change:</strong> A risk goes from <code>2%</code> to <code>4%</code>. Enter (1) absolute change in percentage points and (2) relative percent increase.</p>
                            <div class="s5-grid" style="grid-template-columns: 1fr 1fr; gap: 6px;">
                                <input id="s5-change-absolute" class="s5-input" placeholder="Absolute points" value="${state.changeAnswers.absolute}" ${disabled(state.appliedUnlocked)} />
                                <input id="s5-change-relative" class="s5-input" placeholder="Relative % increase" value="${state.changeAnswers.relative}" ${disabled(state.appliedUnlocked)} />
                            </div>
            ...
            const syncConcreteMission = () => {
                ...
                if (state.concreteMission.alignedDecimals && !state.concreteCompleted) {
                    state.concreteCompleted = true;
                    state.pictorialUnlocked = true;
                    state.decimalFeedback = 'Concrete mission complete. Decimal columns are aligned. Pictorial unlocked. Continue below.';
                }
            };
            ...
            const handleClick = (event) => {
                ...
                if (event.target.closest('#s5-hint-concrete')) {
                    ...
                    if (state.concreteMission.alignedDecimals) {
                        ...
                        state.decimalFeedback = 'Hint: Row A decimal is left of Row B decimal. Keep rotating rows until both dots share the same column.';
                    } else if (decA > decB) {
                        state.decimalFeedback = 'Hint: Row A decimal is right of Row B decimal. Keep rotating rows until both dots share the same column.';
                    } else {
                        state.decimalFeedback = 'Hint: The decimals are already aligned. Click "Check Alignment" to complete the mission.';
                    }
                ...
                if (event.target.closest('#s5-continue-pictorial')) {
                    if (state.fastTrack || state.pictorialUnlocked) {
                        ...
                        state.decimalFeedback = 'Complete the Concrete mission first by aligning both decimal points and checking alignment.';
                    }
                ...
                if (event.target.closest('#s5-check-hop')) {
                    if (state.hopPosition === 1) {
                        state.hopFeedback = 'Correct! Placing the decimal point before the first digit 1 yields 1.44. Abstract unlocked. Continue below.';
                        state.abstractUnlocked = true;
                    } else {
                ...
                if (event.target.closest('#s5-check-sigfigs')) {
                    ...
                    if (correct) {
                        state.sigfigFeedback = 'Correct! Leading zeros are not significant, trapped zeros are significant, and trailing zeros are significant due to the decimal point. 0.05080 has 4 sig figs. Applied unlocked. Continue below.';
                        state.appliedUnlocked = true;
                    } else {
